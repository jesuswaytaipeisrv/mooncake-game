...
- `src/game-audio.js`：Web Audio API 音效合成、瀏覽器偏好保存與自動播放解鎖。
...
音效不進入遊戲狀態或 Firebase。玩家按鍵直接播放本機咀嚼聲；主持台依同步後的 `biteUnits` 增量播放咀嚼聲；主持與玩家各自從狀態轉換判定 85% 衝刺與勝利音效。音效開關偏好只存於各瀏覽器的 `localStorage`。
